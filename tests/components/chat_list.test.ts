...
test('No chat', async () => {
  store.chats = []
  const wrapper: VueWrapper<any> = mount(ChatList, { props: { chat: undefined, filter: '' } } )
...
test('Filter All', async () => {
  const wrapper: VueWrapper<any> = mount(ChatList, { props: { chat: undefined, filter: 'Subtitle' } } )
  expect(wrapper.findAll('.chat')).toHaveLength(10)
})

test('Filter Single', async () => {
  const wrapper: VueWrapper<any> = mount(ChatList, { props: { chat: undefined, filter: '9' } } )
...
test('Multiselect', async () => {
  const wrapper: VueWrapper<any> = mount(ChatList, { props: { chat: undefined, filter: '', selectMode: true } } )
...
test('Context Menu', async () => {
  const wrapper: VueWrapper<any> = mount(ChatList, { ...stubTeleport, props: { chat: undefined, filter: '' } } )

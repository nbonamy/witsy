...
test('Filesystem plugin path validation', async () => {
  ...
  expect(disallowedResult.error).toContain('Access denied')
})
...
test('Filesystem plugin write file - new file', async () => {
  ...
  expect(window.api.file.writeNew).toHaveBeenCalledWith('/tmp/newfile.txt', 'test content')
  expect(result.success).toBe(true)
})

test('Filesystem plugin write file - file exists', async () => {
  const plugin = new FilesystemPlugin(store.config.plugins.filesystem)
  const result = await plugin.execute(context, {
    tool: 'filesystem_write',
  ...
  expect(window.api.file.writeNew).not.toHaveBeenCalled()
})

test('Filesystem plugin disabled', () => {
  const disabledConfig = { enabled: false, allowedPaths: [] }
  const plugin = new FilesystemPlugin(disabledConfig)
  expect(plugin.isEnabled()).toBe(false)
})

test('Filesystem plugin no allowed paths', () => {
  const noPathsConfig = { enabled: true, allowedPaths: [] }
...
test('Filesystem plugin status descriptions', () => {
  ...
  expect(plugin.getCompletedDescription('filesystem_list', { path: '/tmp' }, { items: [1, 2] })).toBe('plugins.filesystem.list.completed')
  expect(plugin.getCompletedDescription('filesystem_read', { path: '/tmp/file.txt' }, { contents: 'test' })).toBe('plugins.filesystem.read.completed')
  expect(plugin.getCompletedDescription('filesystem_write', { path: '/tmp/file.txt' }, {})).toBe('plugins.filesystem.write.completed')
  expect(plugin.getCompletedDescription('filesystem_list', {}, { error: 'test error' })).toBe('plugins.filesystem.error')
})

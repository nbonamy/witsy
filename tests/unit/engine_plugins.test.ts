...
test('Search Plugin Local', async () => {
  ...
  expect(search.getCompletedDescription('', { query: 'query' }, { results: [] })).toBe('plugins.search.completed {"count":0,"query":"query"}')
  expect(search.getCompletedDescription('', { query: 'query' }, { results: [ {} ] })).toBe('plugins.search.completed {"count":1,"query":"query"}')

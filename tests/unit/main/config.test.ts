...
test('Migration: MCP tool name old suffix format', () => {
  ...
  const migrated = migrateSettingsMcpToolNames(app)
  expect(migrated).toBe(6) // 4 in llm.defaults + 2 in prompt + 1 in realtime (transcribe has no suffix) - wait, let me count

...
test('addSitemap handles errors gracefully', async () => {
  ...
  const Loader = (await import('@main/rag/loader')).default
...
test('addFolder creates child file documents', async () => {
  const { listFilesRecursively } = await import('@main/file')

<template>
  <div class="actionbar-wrapper">
    <div class="actionbar" v-if="activeBar == 'standard'">
      <div
        class="action"
        @click="emitEvent('action', 'undo')"
        v-tooltip="'Undo'"
        :class="{ disabled: !undoStack.length }"
      >
        <BIconReplyFill />
      </div>
      ...
      <div
        class="action"
        @click="emitEvent('action', 'redo')"
        v-tooltip="'Redo'"
        :class="{ disabled: !redoStack.length }"
      >
      ...
      <div
        class="action"
        @click="emitEvent('action', 'copy')"
        v-tooltip="'Copy to clipboard'"
      >
        <BIconClipboard v-if="copyState == 'idle'" />
        <BIconClipboardCheck
          style="color: var(--scratchpad-actionbar-active-icon-color)"
          v-else
        />
      </div>
      ...
      <div
        class="action"
        @click="onMagicAction($event, 'spellcheck')"
        v-tooltip="'Spellcheck'"
      >
        <BIconSpellcheck />
      </div>
      ...
      <div
        :class="{ action: true, active: audioState == 'playing', static: true }"
        @click="emitEvent('action', 'read')"
        v-tooltip="'Read aloud'"
      >
    ...
    <div class="actionbar" v-if="activeBar == 'magic'">
      ...
      <div
        class="action"
        @click="onMagicAction($event, 'improve')"
        v-tooltip="'Improve writing'"
      >
        <BIconMortarboard />
      </div>
      ...
      <div
        class="action"
        @click="onMagicAction($event, 'takeaways')"
        v-tooltip="'List key takeaways'"
      >
        <BIconListUl />
      </div>
      ...
      <div
        class="action"
        @click="onMagicAction($event, 'title')"
        v-tooltip="'Suggest a title'"
      >
        <BIconFonts />
      </div>
      ...
      <div
        class="action"
        @click="onMagicAction($event, 'simplify')"
        v-tooltip="'Simplify writing'"
      >
      ...
      <div
        class="action"
        @click="onMagicAction($event, 'expand')"
        v-tooltip="'Expand writing'"
      >
      ...
      <div
        class="action"
        @click="onMagicAction($event, 'complete')"
        v-tooltip="'Complete text'"
      >
        <BIconPencil />
      </div>
    </div>
...
onMounted(() => {
  onEvent("llm-done", () => {
    document
      .querySelectorAll(".action.active")
...
@keyframes active {
  ...
  100% {
    color: var(--scratchpad-actionbar-active-icon-color2);
    transform: scale(1.1);
  }
}
</style>

  ...
  <div class="mcp-servers split-pane">
    <div class="sp-main">
      <template v-if="!selected">
        <List ref="list" :servers="servers" :status="status" :loading="loading" @edit="onEdit" @create="onCreate" @reload="load" @restart="onRestart" @restart-server="onRestartServer" />
      </template>
      <template v-else>
...
const onRestartServer = async (server: McpServer) => {
  loading.value = true
  await nextTick()
  // Use dedicated restart method instead of editServer
  await window.api.mcp.restartServer(server.uuid)
  load()
}

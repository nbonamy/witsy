...
import { deepResearchAgents } from './deepresearch'
import AgentPlugin from '../plugins/agent'
import Message from '../models/message'
import Chat from '../models/chat'

const deepResearchInstructions = `You are a research coordinator, the master orchestrator of deep research investigations.
...
export const useDeepResearchMultiAgent = (config: Configuration, engine: LlmEngine, chat: Chat, opts: AssistantCompletionOpts): Omit<AssistantCompletionOpts, 'model'> => {
  ...
  for (const agent of deepResearchAgents) {
    const agentPlugin = new AgentPlugin(config, agent, agent.engine || engine.getId(), agent.model || opts.model, {
  ...
  if (chat.messages.length === 0) {
    chat.messages.push(new Message('system', deepResearchInstructions))
  } else {
    chat.messages[0].content = deepResearchInstructions
  }

  // force planning to be called
  return {
